...
def plot_comparison(file_paths_set1, file_paths_set2, file_paths_set3, label_set1, label_set2, label_set3):
    ...
    def calculate_cumulative_values(file_paths):
        ...
        for file_path in file_paths:
            ...
            # Pad or truncate the speed/acceleration array to match the max length
            values_padded = np.pad(df_vehicle['speed'].values, (0, max_length - len(df_vehicle)), 'constant')
    ...
    time_set1, cumulative_speed_set1 = calculate_cumulative_values(file_paths_set1)
    time_set2, cumulative_speed_set2 = calculate_cumulative_values(file_paths_set2)
    time_set3, cumulative_speed_set3 = calculate_cumulative_values(file_paths_set3)

    # Plotting the cumulative speed vs time for each set
    # plt.plot(time_set1, cumulative_speed_set1, label=label_set1 + ' Speed')
    plt.plot(time_set2, cumulative_speed_set2, label=label_set2 + ' Speed')
    plt.plot(time_set3, cumulative_speed_set3, label=label_set3 + ' Speed')

    plt.title('Cumulative Speed Comparison over Time')
